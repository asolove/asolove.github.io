...
- For UI cases in particular, a natural fit is [the Concurrent ML primitives](/js/ui/2017/04/02/synchronizable-abstractions.html), which make it easy to express common UI needs like cancellable asynchronous transactions & speculative calculation. They don’t play nicely with the current JavaScript execution model, but…
